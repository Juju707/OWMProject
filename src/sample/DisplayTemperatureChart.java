...
/**
 * Class for displaying temperature chart
 * @author Juju
 * @version 1.0
 */
public class DisplayTemperatureChart implements Observer,DisplayWeather{
    /**
     ...
     * @param scatterChart
     */
    public DisplayTemperatureChart(ScatterChart<Number, Number> scatterChart) {
    ...
    public void update(WeatherParameters weatherParameters) {
        ...
        Platform.runLater(() -> {
            if (weatherParameters!=null) {
                series.getData().add(new XYChart.Data<>(number, weatherParameters.getTemp()));
                number++;}
        });
