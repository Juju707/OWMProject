...
/**
 * Class for displaying pressure chart
 * @author Juju
 * @version 1.0
 */
public class DisplayPressureChart implements Observer,DisplayWeather  {
    /**
     ...
     * @param scatterChart
     */
    public DisplayPressureChart(ScatterChart<Number, Number> scatterChart) {
    ...
    public void update(WeatherParameters weatherParameters) {
        ...
        Platform.runLater(() -> {
            if (weatherParameters!=null) {
                series.getData().add(new XYChart.Data<>(number, weatherParameters.getPressure()));
                number++;}
        });
